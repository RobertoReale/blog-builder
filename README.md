...
Same deployment steps, plus the Keystatic CMS setup below.

---

## 6. Using the Keystatic CMS (Step 7 only)

Keystatic gives you a browser-based editor at `yourdomain.com/keystatic`.
When you save an article, Keystatic commits the `.mdx` file to GitHub.
Vercel detects the push and rebuilds your blog automatically.
...
- **Application name**: `Blog Admin` (or anything)
...
Trigger a new Vercel deployment (push any commit). Then visit:

```
https://yourdomain.com/keystatic
```

Log in with GitHub and start writing.

> **Access control**: only GitHub accounts with **write access** to the repository
> can log into the admin. To grant access, add collaborators in your GitHub repo settings.

### Writing articles with Keystatic

1. Go to `yourdomain.com/keystatic`
2. Click **Articles** → **Create**
3. Fill in the fields (title, description, tags, status, body)
4. Click **Create** — Keystatic commits the `.mdx` to GitHub
...
**Cause**: Node.js 22–24 + `@astrojs/vercel` adapter compatibility issue (esbuild version conflict). This only happens after Step 7 (Keystatic).

**Fix A** — Check that `astro.config.mjs` has this Vite setting:
```js
vite: {
  esbuild: {
    target: 'es2022',
